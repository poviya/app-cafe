...
export class ListComponent implements OnInit {
  ...
  onEdit(course: Course): void {
    this.router.navigate(['/panel/create-course/details/' + course._id]);
  }

  onPromote(course: Course): void {
    this.router.navigate(['/panel/create/promote/' + course._id]);
  }

  onPhotos(course: Course): void {
    this.router.navigate(['/panel/create/media/' + course._id]);
  }
  ...
  onDelete(): void {
    ...
    this.courseService.delete(this.course._id).subscribe(res => {
      console.log(res);
      if(res) {
        this.courses = this.courses.filter((item: Course) => item._id!.split('.')[0] !== this.course._id);
  ...
  onStatus(): void {
    ...
    if (this.course) {
      const data = {
        status: this.course.status ==  'ACTIVE' ?  'SUSPENDED' : 'ACTIVE',
      }
      this.courseService.updateStatus(this.course._id!, data).subscribe( res => {
        if(res)
        {
          this.courses = this.courses.filter((item: Course) => item._id!.split('.')[0] !== this.course._id);
        }
  ...
  onSuspended(course: Course) {

  }

  onActive(course: Course) {
