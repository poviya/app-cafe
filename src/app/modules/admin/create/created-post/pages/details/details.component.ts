...
import { FileMedia, PostCategory, PostMedia, PostSalesUnit } from 'src/app/interfaces';
import { AuthService, PostService, AudioRecordingService, PostCategoryService } from 'src/app/services/index';
...
import { PostSalesUnitService } from 'src/app/services/post-sales-unit.service';
...
export class DetailsComponent implements OnInit {
  ...
  public postSalesUnit: PostSalesUnit[];
  ...
  constructor(
        ...
        private postSalesUnitService: PostSalesUnitService,
  ...
  createFormControls() {
    ...
    this.myform = this.fb.group({
      ...
      PostSalesUnit: ['', Validators.required],
      ...
      price: [5, [Validators.required, NumericValidator]],
      weight: ['GR', Validators.required],
      weightAmount: [5, [Validators.required, NumericValidator]],
  ...
  findAllPostSalesUnit(): void {
    const data = {};
    this.postSalesUnitService.findAll(data).subscribe(res => {
      if(res) 
      this.postSalesUnit = res;
    });
  }
  ...
  onSubmit() {
    ...
    if (this.myform.valid && this.authService.user) {
      ...
      const data = {
        ...
        PostSalesUnit: this.myform.value.PostSalesUnit,
        PostCategory: this.myform.value.PostCategory,
        price: Number(this.myform.value.price),
        weight: this.myform.value.weight,
        weightAmount: Number(this.myform.value.weightAmount),
  ...
  onUpdate(): void {
    ...
    if (this.myform.valid && this.authService.user && this.idPost) {
      ...
      console.log(data);
      this.postService.update2(this.idPost, data).subscribe( res => {
        this.onDiscard();
        this.spinnerService.close();
        console.log('repuesta',res);
      });
  ...
  findOne()
  {
    this.postService.findOne(this.idPost).subscribe(res => {
      this.myform.patchValue({
        PostCategory: res.PostCategory?._id,
        PostSalesUnit: res.PostSalesUnit?._id,
        title: res.title,
        description: res.description,
        price: res.price,
        weight: res.weight,
        weightAmount: Number(res.weightAmount),
        comment: res.comment,
      });
