...
import { FileMedia, } from 'src/app/interfaces';
import { AuthService, PostService, AudioRecordingService, CourseService } from 'src/app/services/index';
...
export class DetailsComponent implements OnInit {
  ...
  progressInfo: any = []
  //Mensaje que almacena la res de las Apis
  message: any = '';
  ...
  createFormControls() {
    this.myform = this.fb.group({
      ...
      whoIsItFor: ['', Validators.required],
      content: this.fb.array([], Validators.required),
      startDate: ['', Validators.required],
      duration: ['', Validators.required],
      modality: ['', Validators.required],
      schedule: ['', Validators.required],
      certifiedHours: ['', Validators.required],
   ...
   // FORM ARRAY
   content() : FormArray {  
    return this.myform.get("content") as FormArray  
  }  
  /*
  newObjetivoEspecifico(): FormGroup {  
  ...
  onSubmit() {
    ...
    if (this.myform.valid && this.authService.user) {
      ...
      const data = {
        ...
        purposes: this.myform.value.purposes,
        benefits: this.myform.value.benefits,
        whoIsItFor: this.myform.value.whoIsItFor,
        content: this.myform.value.content,
        startDate: this.myform.value.startDate,
        duration: this.myform.value.duration,
        modality: this.myform.value.modality,
        schedule: this.myform.value.schedule,
        certifiedHours: this.myform.value.certifiedHours,
      ...
      this.courseService.create(data, this.filesData).subscribe( res => {
        this.onDiscard();
  ...
  onUpdate(): void {
    ...
    if (this.myform.valid && this.authService.user && this.idPost) {
      ...
      console.log(data);
      this.courseService.update2(this.idPost, data).subscribe( res => {
        this.onDiscard();
        this.spinnerService.close();
        console.log('repuesta',res);
      });
  ...
  findOne()
  {
    this.courseService.findOne(this.idPost).subscribe(res => {
      this.myform.patchValue({
        title: res.title,
        description: res.description,
        purposes: res.purposes,
        benefits: res.benefits,
        whoIsItFor: res.whoIsItFor,
        //content: res.content,
        startDate: res.startDate,
        duration: res.duration,
        modality: res.modality,
        schedule: res.schedule,
        certifiedHours: res.certifiedHours,
        price: res.price,
        comment: res.comment,
    ...
    if(res.content.length > 0)
        {
          for (let i = 0; i < res.content.length; i++)
          {
            this.content().push(
              this.fb.group({  
                subject: [res.content[i]['subject'], Validators.required],
              })  
            ); 
          }
        } else {
