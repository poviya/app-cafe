...
export class CourseService {
  ...
  findOne(id: any): Observable<Course> {
    return this.http
      .get<Course>(environment.api + this.url + '/' + id);
      //.pipe(retry(1), catchError(this.handleError));
  }

  findOneSlug(slug: any): Observable<Course> {
    //console.log(slug)
    return this.http.get<Course>(environment.api + this.url + '/slug/' + slug);
  }

  findAll(dataQuery: any, limit: number, offset: number): Observable<any> {
    return this.http.post<Course[]>(environment.api + this.url + `/find-all?limit=${limit}&offset=${offset}`, dataQuery);
  }

  findAllInfinite(dataQuery: any, limit: number, offset: number): Observable<any> {
    return this.http.post<Course[]>(environment.api + this.url + `/find-all-infinite?limit=${limit}&offset=${offset}`, dataQuery);
  }

  findAllCount(dataQuery: any): Observable<object[]> {
    
    return this.http.post<Course[]>(environment.api + this.url + '/find-all-count', dataQuery);
  }

  findAllUser(dataQuery: any, limit: number, offset: number): Observable<any> {
    return this.http.post<Course[]>(environment.api + this.url + `/find-all-user?limit=${limit}&offset=${offset}`, dataQuery, Headers.HttpOptions());
  }

  findAllUserInfinite(dataQuery: any, limit: number, offset: number): Observable<any> {
    return this.http.post<Course[]>(environment.api + this.url + `/find-all-user-infinite?limit=${limit}&offset=${offset}`, dataQuery, Headers.HttpOptions());
  }

  findAllUserCount(dataQuery: any): Observable<object[]> {
    
    return this.http.post<Course[]>(environment.api + this.url + '/find-all-user-count', dataQuery, Headers.HttpOptions());
  }
  ...
  update2(id: string, data: any): Observable<Course> {
    const uri = environment.api + this.url + '/' + id;
    console.log(uri);
    return this.http.put<Course>(uri, data, Headers.HttpOptions());
  }

  updateStatus(id: string, data: any): Observable<Course> {
